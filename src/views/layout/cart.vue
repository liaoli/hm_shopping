<template>
  <div class="cart" v-if="isLogin && $store.state.cart.cartList.length > 0">
    ...
    <!-- 购物车列表 -->
    <div class="cart-list">
      <div class="cart-item" v-for="item in $store.state.cart.cartList" :key="item.goods_id">
        ...
        <div class="info">
          ...
          <span class="bottom">
            ...
            <!-- <div class="count-box">
              ...
              <input @input="(value) => changeCount(value, item.goods_id,item.goods_sku_id)" class="inp" :value="item.goods_num" type="text">
...
export default {
  ...
  created () {
    if (this.isLogin) {
      this.$store.dispatch('cart/getCartAction')// 页面中 dispatch 调用
    }
  },
  methods: {
    toggleCheck (id) {
